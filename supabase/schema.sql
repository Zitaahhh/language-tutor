-- AI Spanish Coach database schema
-- Run this in Supabase SQL Editor for first setup.
...
create policy "daily_tasks_all_own" on public.daily_tasks for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "mistakes_all_own" on public.mistakes;
create policy "mistakes_all_own" on public.mistakes for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
drop policy if exists "telegram_archives_all_own" on public.telegram_archives;
create policy "telegram_archives_all_own" on public.telegram_archives for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
